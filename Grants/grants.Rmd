...
geocodes <- read_csv("../all-geocodes-v2019.csv", 
  ...
  rename_with(~ str_remove(., "\\)"), everything()) %>%
  rename(area_name = "area_name_including_legal/statistical_area_description")
...
rwj <- rwj %>% select(`FIPS`, `Food Environment Index`) %>% rename(
  fips = `FIPS`, 
  rwj_food_environment_index = `Food Environment Index`)
...
direct_intermediated_sales <- read_csv("value_added_data_compiled_2020_05_04.csv", 
...
population_2019 <- read_csv("../co-est2019-alldata.csv") %>% clean_names() %>% filter(sumlev=='050') %>% select(sumlev:ctyname, popestimate2019) 

# Combine state and county codes to create a FIPS code
population_2019 <- population_2019 %>% mutate(
...
direct_intermediated_sales <- left_join(direct_intermediated_sales, population_2019)

direct_intermediated_sales <- direct_intermediated_sales %>% mutate(
  direct_sales_19_per_capita = direct_sales_17/popestimate2019, 
  intermediated_sales_19_per_capita = intermediated_sales_17/popestimate2019) %>% select(
    fips, direct_sales_19_per_capita, intermediated_sales_19_per_capita)
...
grants <- grants %>% rename(
  ...
  direct_sales_per_capita = direct_sales_19_per_capita, 
  intermediated_sales_per_capita = intermediated_sales_19_per_capita)
